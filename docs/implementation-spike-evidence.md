...
## 8. V0 post-extraction live acceptance

After provider, auth-node, and observability extraction, the integration test explicitly constructed `fileCodexAuthStore(undefined, { cwd: process.cwd(), env: process.env })` and passed it through `codexNodeAdapter`. A status-only preflight confirmed that the resolved store was `.providers/.codex/auth.json` and contained tokens; no account or token value was printed.

Exactly one filtered `gpt-5.6-luna` stream ran. It passed in 3.149 seconds while the other three live cases remained skipped. The support-safe report recorded one sent attempt with complete positive input/output/total usage, zero possibly billed attempts without usage, matching run/model/attempt correlation, a provider request ID matching both attempt report and terminal observation, complete delivery, and healthy exporter state. Observation content policy was `none`, the adapter had no exact-wire request observer, and the test proved the observation stream contained neither prompt, system instruction, nor response text.

The report is emitted with mode `0600` to the ignored `.temp/live-acceptance/codex-luna-safe-report.json` by default, or to `AI_AGENT_SDK_LIVE_REPORT_DIR` for CI artifact upload. It contains only support-safe identifiers, counters, delivery/health state, and event names; credentials and wire content are never written.
