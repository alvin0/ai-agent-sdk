...
7. `eventsource-parser@4.1.0` remains exact-pinned only as a direct dependency of `@ai-agent-sdk/provider-http`. The dependency policy retains it after the owned candidate failed the mandatory throughput gate; replacement requires a new fully qualified decision.
