...
describe('workspace compatibility runtime identity', () => {
  it('re-exports single core, agent, transport, and protocol package instances', async () => {
    ...
    const codexCompatibility = await import(pathToFileURL(resolve(root, 'dist/codex.js')).href)
    ...
    const mcpNodeCompatibility = await import(pathToFileURL(resolve(root, 'dist/mcp-node.js')).href)
    const skillCompatibility = await import(pathToFileURL(resolve(root, 'dist/skill-filesystem.js')).href)
    ...
    const authEnv = await import(pathToFileURL(resolve(root, 'packages/auth-node/dist/env.mjs')).href)
    ...
    expect(compatibility.apiKeyFromEnv).toBe(authEnv.apiKeyFromEnv)
