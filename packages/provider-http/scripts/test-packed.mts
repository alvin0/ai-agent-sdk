...
const providerTarball = pack(packageRoot, artifacts)
const temporaryRoot = mkdtempSync(join(tmpdir(), 'ai-agent-sdk-provider-http-pack-'))
try {
  ...
  process.stdout.write(`packed provider-http runtime matrix passed: ${relative(workspaceRoot, providerTarball)}\n`)
} finally {
