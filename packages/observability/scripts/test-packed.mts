...
const observabilityTarball = pack(packageRoot, artifacts)
const temporaryRoot = mkdtempSync(join(tmpdir(), 'ai-agent-sdk-observability-pack-'))
try {
  ...
  for (const runtime of ['standards', 'browser', 'worker']) {
    ...
    run('npm', [
      ...
      coreTarball, observabilityTarball,
    ], consumer)
  ...
  process.stdout.write(`packed observability runtime matrix passed: ${relative(workspaceRoot, observabilityTarball)}\n`)
} finally {
...
async function testBrowser(consumer: string): Promise<void> {
  ...
  try {
    ...
    await page.waitForFunction(() => '__observabilityFixture' in globalThis)
    assertFixture(await page.evaluate(() => (
      globalThis as typeof globalThis & { __observabilityFixture: unknown }
    ).__observabilityFixture), 'browser')
  } finally {
    await browser.close()
...
function assertFixture(value: unknown, runtime: string): void {
  ...
  if (result.eventCount !== 2 || result.logLevel !== 'warn' || result.traceName !== 'sdk.model.call'
    || result.metricCount !== 4 || result.complete !== true || result.healthy !== 'healthy'
    || result.safe !== true || result.buffer !== 'undefined' || result.process !== 'undefined') {
