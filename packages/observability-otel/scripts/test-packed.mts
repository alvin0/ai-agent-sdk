...
try {
  ...
  process.stdout.write(`packed observability-otel runtime matrix passed: ${relative(workspaceRoot, otelTarball)}\n`)
} finally {
...
async function testBrowser(consumer: string): Promise<void> {
  ...
  try {
    ...
    try { await page.waitForFunction(() => '__observabilityOtelFixture' in globalThis) }
    catch (error) {
    ...
    assertFixture(await page.evaluate(() => (
      globalThis as typeof globalThis & { __observabilityOtelFixture: unknown }
    ).__observabilityOtelFixture), 'browser')
  } finally {
    await browser.close()
