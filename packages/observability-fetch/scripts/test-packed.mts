...
try {
  ...
  process.stdout.write(`packed observability-fetch runtime matrix passed: ${relative(workspaceRoot, fetchTarball)}\n`)
} finally {
...
async function testBrowser(consumer: string): Promise<void> {
  ...
  try {
    ...
    await page.waitForFunction(() => '__observabilityFetchFixture' in globalThis)
    assertFixture(await page.evaluate(() => (
      globalThis as typeof globalThis & { __observabilityFetchFixture: unknown }
    ).__observabilityFetchFixture), 'browser')
  } finally {
    await browser.close()
