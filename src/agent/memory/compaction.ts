...
export class ContextCompactor {
  ...
  private async summarize(
    ...
    const iterator = prepared.stream(request)[Symbol.asyncIterator]()
    let exhausted = false
    let events = 0
    let responseBytes = 0
    let sawFinish = false
    try {
      ...
      if (!exhausted) {
        ...
        if (close !== undefined) {
          ...
          if (!await waitForSettlement(closing, policy.teardownTimeoutMs)) {
            throw codedError(
...
function newCompactionId(): string {
  return globalThis.crypto?.randomUUID?.() ?? `compact-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`
}
