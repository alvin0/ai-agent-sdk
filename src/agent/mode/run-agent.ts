...
export const DEFAULT_AGENT_CALL_CONFIG: Readonly<CallConfig> = Object.freeze({
  provider: 'codex',
  model: 'gpt-5.6-luna',
...
interface AgentRunCommon extends Omit<RunTurnOptions, 'bounds' | 'commentary' | 'config' | 'hooks' | 'system' | 'tools'> {
  /** Omit to use Codex `gpt-5.6-luna` with medium reasoning effort. */
...
async function driveAgent(
  ...
  const turnOptions: RunTurnOptions = {
    ...
    config: options.config ?? DEFAULT_AGENT_CALL_CONFIG,
