...
const MAX_MEMBER_NAME_LENGTH = 128
...
function messageToolSchema(messageDescription: string): Record<string, unknown> {
  return {
    type: 'object',
    properties: {
...
function parseMessageTool(raw: unknown): { target: string; message: string } {
...
function parseWaitTool(raw: unknown): { targets: readonly string[] } {
...
function emptyObject(raw: unknown, label: string): Record<string, never> {
...
function newTeamId(): string {
  return globalThis.crypto?.randomUUID?.()
    ?? `team-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`
}

function newMessageId(): string {
  return globalThis.crypto?.randomUUID?.()
    ?? `a2a-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`
}
