...
`ApprovalDecision` là `'allow' | 'deny' | 'abort'`, và `resolve` trả lời theo
`approvalRequestId` do SDK cấp — không bao giờ theo id lời gọi của nhà cung cấp,
thứ mà một lần phát lại có thể lặp lại.
...
- [Durable Execution](/vi/03-tools/durable-execution) — ghi sổ phê duyệt xuyên qua khởi động lại
