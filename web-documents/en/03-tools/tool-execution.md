...
An interceptor is a **named object** with three optional phases — `before`
(allow / deny / ask), `around` (wrap execution), `after` (accept / replace /
block the result). `next()` takes no arguments; the call arrives as the first
parameter.
...
`createToolExecutionInterceptor` is a ready-made `around` adapter for remote
backends and durable operations. See
